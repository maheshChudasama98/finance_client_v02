...
import { CustomTable, } from 'src/components/CustomComponents';
...
export default function Index() {
    ...
    const StatusChange = (action, value, id) => {
        ...
        dispatch(LabelActionService({ [action]: value, LabelId: id }, () => {
            setApiFlag(!apiFlag);
        }));
    };
    ...
    useEffect(() => {
        if (!displayFlag) {
            ...
            dispatch(LabelsFetchListService(payLoad, (res) => {
                if (res?.status) {
    ...
    const tableSetData = accountsList.map((item, index) => ({
        ...
        Label: <Typography variant="normal">
            {item?.LabelName || ""}
        </Typography>,
        Color:
            <Typography variant="normal">
        ...
        Used: (loadingSwitch[item?.LabelId] && loadingSwitch?.action === "isUsing") ? (
            <ButtonLoader />
        ) : (
            <Switch
        ...
        Active: (loadingSwitch[item?.LabelId] && loadingSwitch?.action === "isActive") ? (
            <ButtonLoader />
        ) : (
            <Switch
