...
export default function SubForm({ backAction, editObject, CategoryId, Color, deleteAction }) {
  ...
  const ActionSubmit = (values) => {
    ...
    dispatch(
      SubCategoryModifyService(values, (res) => {
        if (res?.status) {
          backAction();
        }
      ...
      {(props) => {
        ...
        return (
          <Form>
            ...
            >
              <Grid item xs={12}>
                <CustomIconPicker required formik={props} label="Icon" field="Icon" color={Color} />
              </Grid>
              <Grid item xs={12} md={6}>
                <TextFieldForm formik={props} label="Category" field="SubCategoriesName" />
              </Grid>
              ...
              <Grid xs={12}>
                <Box sx={{ float: 'right', display: 'flex' }}>
                  {editObject?.SubCategoryId && editObject?.isPrimitive === 1 && (
                    <Button
                      variant="contained"
                      sx={{ marginX: 1 }}
                      onClick={() => {
