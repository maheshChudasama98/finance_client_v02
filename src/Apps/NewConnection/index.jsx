...
import { RemoveMeterService, StatusModifyService, FetchMeterListService, } from 'src/Services/Meter.Services';
...
import FormDetails from './MeterForm';
...
export default function Index() {
    ...
    useEffect(() => {
        if (!displayFlag) {
            ...
            dispatch(FetchMeterListService(payLoad, (res) => {
    ...
    const DeleteAction = () => {
        ...
        dispatch(RemoveMeterService({ MeterId: deleteObject?.Meter_Id, }, () => {
            setOpen(false);
            const payLoad = {
                Page: 1,
                PageSize: 30,
    ...
    const StatusChange = (item, Status) => {
        setStatusChangeButtonLoader((prev) => ({ ...prev, [item?.Meter_Id]: true }));
        dispatch(StatusModifyService({ MeterId: item?.Meter_Id, Status }, () => {
            setOpen(false);
            const payLoad = {
                Page: 1,
                PageSize: 30,
            ...
            dispatch(FetchMeterListService(payLoad, (res) => {
                setMeterList(res?.list);
                setCountObject(res?.count);
                setStatusChangeButtonLoader((prev) => ({ ...prev, [item?.Meter_Id]: false }));
            }))
        }))
    }
                                                        ...
                                                        <Box sx={{
                                                            ...
                                                            (
                                                                <MeterList
                                                                    ...
                                                                    item={item}
                                                                    meterList={meterList}
                                                                    statusChangeButtonLoader={statusChangeButtonLoader[item?.Meter_Id]}
                                                                    editAction={(value) => {
                                                                        setEditObject(value);
                                                                        setDisplayFlag(!displayFlag);
                                                                    }}
