...
import { CustomSelect, CustomPerformance } from 'src/components/CustomComponents';
...
export default function Performance({ AccountId }) {
  ...
  }, [AccountId, duration, dispatch]);

  const tableSetData = list?.map((item, index) => ({
    key: index,
    value: item,
    child: <CustomPerformance list={item.child} />,
    duration: (
            ...
            {item?.child && (
              <Box>
                ...
                <Box sx={{ maxHeight: '500px', overflow: 'auto' }}>
                  <CustomPerformance list={item.child} />
                </Box>
              </Box>
            )}
          </AccordionDetails>
