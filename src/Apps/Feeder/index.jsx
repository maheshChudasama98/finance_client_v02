...
import { RemoveFeederService, FetchFeederListService, } from 'src/Services/Feeders.Services';
...
import FormDetails from './FeederForm';
...
export default function Index() {
    ...
    useEffect(() => {
        if (!displayFlag) {
            ...
            dispatch(FetchFeederListService(payLoad, (res) => {
                setFeederList(res);
    ...
    const DeleteAction = () => {
        dispatch(RemoveFeederService({ Feeder_Id: deleteObject?.Feeder_Id, }, () => {
            setOpen(false);
            const payLoad = {
                Page: 1,
                PageSize: 30,
        ...
        <Box sx={{ paddingX: { xs: 0, sm: 2 } }}>
            <Card>
                <CardHeader
                    title="Feeders"
                    // subheader=""
                    action={ReduxUserRole !== ContractorID &&
                        <Button
