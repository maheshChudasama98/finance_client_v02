...
export default function AccountMenu({ color = 'primary', defaultValue, callBack, iconWidth = 40 }) {
  ...
  return (
    <>
      <Box
        sx={{ display: 'flex', alignItems: 'center', textAlign: 'center', margin: 0, padding: 0 }}
      >
        <Tooltip title="select icon">
          ...
          >
            <Avatar sx={{ width: iconWidth, height: iconWidth, background: color }}>
              {selectedIcon ? (
                <FontAwesomeIcon icon={selectedIcon || ''} background={color} />
              ) : (
                <FontAwesomeIcon icon={faQuestion || ''} background={color} />
              )}
            </Avatar>
          </IconButton>
        </Tooltip>
